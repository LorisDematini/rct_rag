...
def build_tfidf_analysis(studies, sections=['SUMMARY','SCIENTIFIC JUSTIFICATION','OBJECTIVES','METHODOLOGY','PROCEDURE','ELIGIBILITY','DATA MANAGEMENT','STATISTICAL']):
    ...
    for section in sections:
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(f'{output_dir}/tfidf_top_terms_{section.lower()}_prepro.png')
            plt.close()
            
            # Sauvegarder les fréquences en CSV
            word_freq.to_csv(f'{output_dir}/tfidf_frequencies_{section.lower()}_prepro.csv', index=False)
