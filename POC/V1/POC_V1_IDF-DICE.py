...
def search(query, vectorizer, tfidf_matrix, labels):
    ...
    distances = np.array([dice(binary_query_vector.toarray()[0], binary_tfidf_matrix[i].toarray()[0]) for i in range(binary_tfidf_matrix.shape[0])])
...
def main():
    ...
    while True:
        ...
        for (study_id, section), dist in results:
            print(f"Étude : {study_id}, Section : {section}, Distance de DICE : {dist:.4f}")
